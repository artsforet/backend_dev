...
export class PlaylistController {
  ...
  async addMusicToPlaylist(
    ...
    return this.playlistService.addMusicToPlaylist(userId, playlistId, musicId);
  }
  ...
  async addSongToPlaylist(
    ...
    @Body('isPlaying') isPlaying: boolean,
  ) {
    // const user = await this.authService.Validate(req); // 예시: AuthService에서 사용자를 인증하고 사용자 정보를 반환하는 메서드
    // console.log('user', user); // 사용자 정보가 제대로 반환되는지 확인
    // if (!user) {
    //   throw new UnauthorizedException(); // 사용자 정보가 없으면 권한 없음 예외를 던짐
    // }
    // const userId = user.id; // 사용자 ID를 가져옴
